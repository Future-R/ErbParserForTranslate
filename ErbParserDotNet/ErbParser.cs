...
public class ERBParser
{
    ...
    public void ParseFile(string filePath)
    {
        ...
        for (int i = 0; i < lineList.Count; i++)
        {
            ...
            else if (lineString.StartsWith("CALL ") || lineString.StartsWith("TRYCALL") || lineString.StartsWith("TRYCCALL") || lineString.StartsWith("JUMP ") || lineString.StartsWith("CALL\t"))
            {
                int spIndex = Tools.GetSpaceIndex(lineString);
                string rightValue = lineString.Substring(spIndex).Trim();
            ...
            else if (lineString.StartsWith("CALLF"))
            {
                int spIndex = Tools.GetSpaceIndex(lineString);
                string rightValue = lineString.Substring(spIndex).Trim();
            ...
            else if (lineString.StartsWith("#DIM"))
            {
                ...
                else
                {
                    ...
                    var leftEnumer = leftValue.Split(' ')
                        .Where(arg => !string.IsNullOrWhiteSpace(arg.Replace("<Tab>", "")));
